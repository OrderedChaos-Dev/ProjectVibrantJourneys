...
public class JuniperTreeFeature extends Feature<BaseTreeFeatureConfig>{
	...
	public JuniperTreeFeature(Codec<BaseTreeFeatureConfig> CODEC) {
		super(CODEC);
	}
	...
	private static boolean isDirtOrFarmlandAt(IWorldGenerationBaseReader p_236418_0_, BlockPos p_236418_1_) {
		return p_236418_0_.isStateAtPosition(p_236418_1_, (p_236409_0_) -> {
			...
			for(Block terracotta : JuniperSaplingBlock.TERRACOTTA_BLOCKS)
				if(block == terracotta)
					return true;
			return isDirt(block) || block == Blocks.FARMLAND || block instanceof SandBlock;
		});
	}
