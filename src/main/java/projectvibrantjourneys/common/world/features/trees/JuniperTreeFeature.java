...
public class JuniperTreeFeature extends AbstractSmallTreeFeature<TreeFeatureConfig> {
	...
	public boolean func_225557_a_(IWorldGenerationReader world, Random rand, BlockPos pos, Set<BlockPos> logs, Set<BlockPos> leaves, MutableBoundingBox box, TreeFeatureConfig config) {
		...
		if (!optional.isPresent()) {
			...
			for(int i = 0; i < height; i++) {
				this.func_227216_a_(world, rand, placement, logs, box, config);
				...
				if(i > height / 2) {
					for(Direction d : Direction.values()) {
						if(rand.nextFloat() <= 0.15)
							...
							this.func_227219_b_(world, rand, placement.offset(d), leaves, box, config);
					}
				}
	...
	private boolean placeBerries(IWorldGenerationReader world, BlockPos pos, Set<BlockPos> leaves, MutableBoundingBox box) {
		if (!isAirOrLeaves(world, pos) && !isTallPlants(world, pos) && !isWater(world, pos)) {
			...
			this.func_227217_a_(world, pos, PVJBlocks.berried_juniper_leaves.getDefaultState(), box);
