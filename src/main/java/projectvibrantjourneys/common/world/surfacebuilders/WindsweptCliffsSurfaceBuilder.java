...
public class WindsweptCliffsSurfaceBuilder extends SurfaceBuilder<SurfaceBuilderConfig> {
	public WindsweptCliffsSurfaceBuilder(Codec<SurfaceBuilderConfig> CODEC) {
		super(CODEC);
	}
	...
	public void apply(Random rand, IChunk chunk, Biome biome, int x, int z, int startHeight, double noise,
		...
		if(f >= 0.85F) {
			...
			SurfaceBuilder.DEFAULT.apply(rand, chunk, biome, x, z, startHeight, noise, defaultBlock,
					defaultFluid, seaLevel, seed, PVJSurfaceBuilders.CONFIG_MOSS);
		} else  {
			SurfaceBuilder.DEFAULT.apply(rand, chunk, biome, x, z, startHeight, noise, defaultBlock,
					defaultFluid, seaLevel, seed, PVJSurfaceBuilders.CONFIG_COBBLESTONE);
		}
	}
}
