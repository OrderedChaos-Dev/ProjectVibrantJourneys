...
public class EntityPVJBoat extends EntityBoat
{
    ...
    private EntityPVJBoat.Status status;
    private EntityPVJBoat.Status previousStatus;
    private double lastYd;

    public EntityPVJBoat(World worldIn)
    {
        super(worldIn);
    ...
    public EntityPVJBoat(World worldIn, double x, double y, double z)
    {
        this(worldIn);
    ...
    {
        if (entityIn instanceof EntityPVJBoat)
        {
    ...
    public void onUpdate()
    {
        ...
        if (this.status != EntityPVJBoat.Status.UNDER_WATER && this.status != EntityPVJBoat.Status.UNDER_FLOWING_WATER)
        {
    ...
    {
        EntityPVJBoat.Status EntityPVJBoat$status = this.getUnderwaterStatus();

        if (EntityPVJBoat$status != null)
        {
        ...
        {
            return EntityPVJBoat.Status.IN_WATER;
        }
        else
        {
            ...
            {
                this.boatGlide = f;
                return EntityPVJBoat.Status.ON_LAND;
            }
            else
            {
                return EntityPVJBoat.Status.IN_AIR;
            }
        }
    }
    ...
    @Nullable
    private EntityPVJBoat.Status getUnderwaterStatus()
    {
        AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
        ...
        return flag ? EntityPVJBoat.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion()
    {
        ...
        else
        {
            ...
            else if (this.status == EntityPVJBoat.Status.UNDER_WATER)
            {
                d2 = 0.009999999776482582D;
                this.momentum = 0.45F;
            }
            else if (this.status == EntityPVJBoat.Status.IN_AIR)
            {
                this.momentum = 0.9F;
            }
            else if (this.status == EntityPVJBoat.Status.ON_LAND)
            {
    ...
    protected void updateFallState(double y, boolean onGroundIn, IBlockState state, BlockPos pos)
    {
        ...
        if (!this.isRiding())
        {
            if (onGroundIn)
            {
                ...
                {
                    if (this.status != EntityPVJBoat.Status.ON_LAND)
                    {
