...
public class WorldGenMudlandsTree extends WorldGenAbstractTree
{
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        for (i = rand.nextInt(5) + 5; worldIn.getBlockState(position.down()).getMaterial() == Material.WATER; position = position.down())
        {
            ;
        }
        ...
        if (position.getY() >= 1 && position.getY() + i + 1 <= 256)
        {
            ...
            else
            {
                ...
                boolean isSoil = state.getBlock() == PVJBlocks.mud;
