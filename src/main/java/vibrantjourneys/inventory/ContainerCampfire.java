...
{
    private final IInventory campfire;
    private int cookTime;
    private int totalCookTime;
    private int furnaceBurnTime;
    private int currentItemBurnTime;

    public ContainerCampfire(InventoryPlayer playerInventory, IInventory campfireInventory)
    {
        this.campfire = campfireInventory;
        this.addSlotToContainer(new Slot(campfireInventory, 0, 56, 17));
        this.addSlotToContainer(new SlotCampfireFuel(campfireInventory, 1, 56, 53));
    ...
    public void detectAndSendChanges()
    {
        ...
        for (int i = 0; i < this.listeners.size(); ++i)
        {
            ...
            if (this.cookTime != this.campfire.getField(2))
            {
                icontainerlistener.sendWindowProperty(this, 2, this.campfire.getField(2));
            }

            if (this.furnaceBurnTime != this.campfire.getField(0))
            {
                icontainerlistener.sendWindowProperty(this, 0, this.campfire.getField(0));
            }

            if (this.currentItemBurnTime != this.campfire.getField(1))
            {
                icontainerlistener.sendWindowProperty(this, 1, this.campfire.getField(1));
            }

            if (this.totalCookTime != this.campfire.getField(3))
            {
                icontainerlistener.sendWindowProperty(this, 3, this.campfire.getField(3));
            }
        }

        this.cookTime = this.campfire.getField(2);
        this.furnaceBurnTime = this.campfire.getField(0);
        this.currentItemBurnTime = this.campfire.getField(1);
        this.totalCookTime = this.campfire.getField(3);
    }
