...
public class GravelPitFeature extends Feature<NoneFeatureConfiguration> {
    ...
    private BlockState getGravelBlock(RandomSource random) {
        float chance = random.nextFloat();
        if (chance < 0.02F) {
            return PVJBlocks.GILDED_GRAVEL.get().defaultBlockState();
        } else if (chance < 0.07F) {
            return PVJBlocks.FERROUS_GRAVEL.get().defaultBlockState();
        }
